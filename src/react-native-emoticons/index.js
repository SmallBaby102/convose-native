...
import { TouchableWithoutFeedbackArea } from "../convose-app/components/ModalWrapper"
const { height, width } = Dimensions.get("window")
import {
  trimmedEmojiData,
  trimmedConsiceEmojiData,
  emojiShortcuts,
} from "./emoji"
import { withTheme } from "styled-components"
require("string.fromcodepoint")

const tabBarUnderlineStyle = { backgroundColor: "#fc7d30", height: 2 }
const categories = [
  "People",
  "Nature",
  "Foods",
...
class Emoticons extends React.Component {
  ...
  componentDidUpdate() {
    ...
    Animated.timing(this.state.wvPosition, {
      duration: 300,
  ...
  _classify() {
    let filteredData = emojiData.filter(
      (e) => !_.includes(filters, e.short_name)
    )
    ...
    if (this.props.concise) {
      filteredData = emojiData.filter((e) =>
        _.includes(choiceness, e.short_name)
      )
      const temp = []
      ...
      _.each(choiceness, (value) => {
        ...
        if (one[0]) this.state.data.push(one[0])
      })
    } else {
      this.state.data = _.mapValues(groupedData, (group) =>
        group.map((value) => {
          return {
            code: this._charFromCode(value.unified),
            name: value.short_name,
  ...
  render() {
            ...
            {ge.map((value, key) => {
              if (
                ...
                return (
                  ...
                  >
                    <Text style={styles.emoji}>{value.code}</Text>
                  </TouchableHighlight>
                )
            })}
            {(this.props.asyncRender &&
    ...
    let groupsView = []
    const plusButton = (
      <View tabLabel={"plus"} style={styles.cateView} key={"0_plus"} />
    )

    const histroyView = group(the.state.history)
    const history = (
      <View tabLabel={"history"} style={styles.cateView} key={"0_history"}>
        <ScrollableTabView
    ...
    return (
      <>
        {this.props.show && (
          <TouchableWithoutFeedback onPress={() => this._onBackspacePress()}>
            <TouchableWithoutFeedbackArea />
          </TouchableWithoutFeedback>
        )}
        {!this.state.showWV ? (
          <Animated.View
          ...
          >
            <ScrollableTabView
              ...
              renderTabBar={() => (
                <TabBar
                  {...this.props}
                  onPlusPress={this._onPlusPress.bind(this)}
                />
              )}
          ...
          <Animated.View
            style={[styles.wvContainer, { bottom: this.state.wvPosition }]}
          >
...
export default withTheme(Emoticons)
export {
  stringify,
  parse,
  splitter,
  trimmedEmojiData as emojiData,
  EmojiSearcher,
  trimmedConsiceEmojiData as conciseData,
  emojiShortcuts,
}
